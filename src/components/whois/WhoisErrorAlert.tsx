...
export const WhoisErrorAlert = ({ error, domain, onRetry }: WhoisErrorAlertProps) => {
  ...
  const getErrorMessage = (error: string) => {
    if (error.includes("404") || error.includes("Request failed with status code 404")) {
      return "找不到WHOIS API服务。请确保API服务已正确部署并可访问。如果您在本地运行，请确保API服务器已启动。";
    ...
    } else if (error.includes("whoiser") || error.includes("not a function")) {
      return "WHOIS客户端查询工具出错，正在尝试使用备用方式查询。";
    }
    return error;
  };
